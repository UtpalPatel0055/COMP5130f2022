...
def getSingleNewAssessment(id=None):
    user_id = get_jwt_identity()["id"] if get_jwt_identity()[
        "id"] else get_jwt_identity()["user_id"]
    new_assessment = NewAssessmentModel.get_by_id(id=id, user_id=user_id)
    if new_assessment:
...
@new_assessment.get("/question/merge/<int:id>")
def getAllInOne(id=None):
    if id:
        assessments = NewAssessmentModel.get_by_id(1, 1)
        if assessments:
            questions = QuestionsModel.get_all_by_id(1, assessments['id'])
            return jsonify({"status": True, "message": "Found", "data": {"assessment": assessments, "questions": questions}})
    else:
        return jsonify({"status": False, "message": "assessment id is missing", "data": []})
