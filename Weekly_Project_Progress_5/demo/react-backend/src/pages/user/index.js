...
const index = (props) => {
	...
	const  handleEditAssessment=(id = undefined, assessmentName = undefined) => {
		
		props.history.push("/new-assessment/edit/")
		alert(id)
	}
	...
	const  handleCopyAssessment=(id = undefined, assessmentName = undefined) => {
				...
				<Row className="mt-3">
					...
					<Col className="my-3 p-1 assess-btn">
						...
						<div className="ml-auto mr-md-5 p-1">
							{Auth.hasAccess() ? (<Button type="link" href="/new-assessm	ent" variant="success" onMouseEnter={() => { setSpin(true) }} onMouseLeave={() => setSpin(false)}><FontAwesomeIcon icon={faPlusCircle} className="mx-2" spin={spin} />Create New Assessment</Button>) : ''}
						</div>
					</Col>
											...
											<tr key={index}>
												...
												<td>
													<div className="d-flex justify-content-around">
														...
														<button className="actions-btn"><FontAwesomeIcon icon={faEdit} onClick={() => handleEditAssessment(item.id, item.assessment_name)} /></button>
														<button className="actions-btn"><FontAwesomeIcon icon={faClone} onClick={() => handleCopyAssessment(item.id, item.assessment_name)} /></button>
														<button className="actions-btn"><FontAwesomeIcon icon={faArrowRight} onClick={() => handleInviteCandidate(item.id, item.assessment_name)} /></button>
													</div>
												</td>
											</tr>
										)
